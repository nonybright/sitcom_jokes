    ...
    return StreamBuilder(
        ...
        builder: (context, currentUserSnapShot) {
          return StreamBuilder<LoadStatus>(
              ...
              stream: (widget.jokeType == JokeType.image)
                  ? _movieBloc.imageLoadStatus
                  : _movieBloc.textLoadStatus,
              builder: (context, loadSnapshot) {
                ...
                return StreamBuilder<UnmodifiableListView<Joke>>(
                  initialData: null,
                  stream: (widget.jokeType == JokeType.image)
                      ? _movieBloc.imageJokes
                      : _movieBloc.textJokes,
                  builder: (context, jokeSnapshot) {
                    return Scaffold(
                      backgroundColor: (widget.jokeType == JokeType.image)
                          ? Colors.black
                          : null,
                      appBar: AppBar(
  ...
  _jokeOptionsRow(Joke joke, User currentUser, BuildContext context) {
    ...
    return Row(
      ...
      children: <Widget>[
        ...
        _jokeActionBox(
            'Favorite', Icons.favorite, (joke != null) ? joke.isFaved : false,
            () {
          if (currentUser != null) {
            _movieBloc.toggleFavorite(joke.id, jokeType, currentUser);
          } else {
            Scaffold.of(context).showSnackBar(SnackBar(
              content: Text('Login to add Favorites'),
            ));
          }
        }),
